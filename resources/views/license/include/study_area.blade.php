...
<?php
if($subject->license_phase !== 1) {continue;}
$first = true;
$active = '';
...
<div class="card-title center py-10">
 ...
 <br />
 <a class="f14" href="#descPart{!!Auth::user()->study_area_history_page_id!!}">前回学習した項目までスキップ</a>
 @elseif( Auth::user()->study_area_history_page_url and Auth::user()->study_area_history_page_url != $license_examination_subject_id )
 <br />
 <a class="f14" href="/license/1/getLicensestudyArea?license_examination_subject_id={!!Auth::user()->study_area_history_page_url!!}">前回学習した科目までスキップ</a>
 @endif
 @endif
</div>
...
$count = count($tmp);
?>
<div id="descPart{!!$desc_part->id!!}" class="card-body row border-bottom @if($first) border-top @endif pb-2 mb-2">
  <div class="col-sm-10">
  ...
  <div class="col-sm-2">
    <a href="javascript:void(0)" data-toggle="modal" data-target="#modelmemoRegi" data-whatever="{!!$desc_part->id!!}" >メモ追加</a>@if($desc_part->wiki_url or $desc_part->literature_url)<br />@endif
