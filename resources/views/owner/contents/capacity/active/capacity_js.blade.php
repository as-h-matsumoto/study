...
function createCapacity(capacity, use_number){
    ...
    if(use_number) main += '<p class="text-success">[ご利用アクティブ]</p>';
    main += '<h4 ><strong>' + truncate(capacity.name, 60) + '</strong></h4>';
    main += '</div>';
    main += '<div class="p-2 center">';
    if(capacity.person>1) main += '<span class=" pr-1">許容人数:' + capacity.person + '名</span>';
    var gobi = (capacity.type===3) ? 'コート' : '台';
    if(use_number){
        if(capacity.number>1) main += '<span class="pr-1">利用数:' + use_number + gobi + '</span>';
    }else{
        if(capacity.number>1) main += '<span class="pr-1">数:' + capacity.number + gobi + '</span>';
    }
    var price_time = (capacity.type===8) ? '(一日)' : '('+capacity.time+'分)';
    main += '<br /><span class="f14 pr-2 text-blue-900">&yen;' + capacity.price + '<span class="">'+price_time+'</span></span>';
    ...
    main += truncate(capacity.description, 300);
    main += '</strong></a>';
    }
    main += '</div>';
    tmpPc += main;
