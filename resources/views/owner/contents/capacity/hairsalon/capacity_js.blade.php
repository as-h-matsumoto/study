...
function createCapacity(capacity, use_number){
    ...
    main += '<h4><strong>' + truncate(getCapacityType('{!!UtilYoyaku::getNewMenuSenMonTenSummary($content->service)!!}', capacity.type, null), 60) + '</strong></h4>';
    main += '</div>';
    main += '<div class="p-2 center">';
    if(capacity.private===1){main += '<span class="pr-2 text-info ">個室</span>'};
    if(use_number){
        main += '<span class=" pr-1">:' + use_number + '席利用</span>';
    }else{
        main += '<span class="">数:' + capacity.number + '席</span>';
    }
    @if($GLOBALS['urls'][1]==='owner')
    if(capacity.delete_flug===1){main += '<br /><span class="pr-2 text-warning ">削除予定</span>'};
    @endif
    main += '</div>';
    main += '<div class=" pl-1 pb-2 center center text-blue-grey-800" style="min-height:50px;">';
