...
function createCapacity(capacity, use_number){
    ...
    if(use_number) main += '<p class="text-success">[ご利用スタジオ]</p>';
    ...
    if(capacity.least_time>=1) main += '<br /><span class=" pr-1">最低利用' + capacity.least_time + 'h~</span>';
    if(use_number){
        main += '<span class="">利用スタジオ数:' + use_number + '室</span>';
    }else{
        if(capacity.number>1) main += '<span class=" pr-1">スタジオ数:' + capacity.number + '</span>';
    }
