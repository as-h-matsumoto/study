...
function createMenu(menu, use_number){
    ...
    if(isset(menu.pic)){
        ...
        pic_src += '/storage/global/img/{!!UtilYoyaku::getNewMenuSenMonTenSummary($content->service)!!}'+Math.floor((Math.random() * 10) + 1)+'_250.jpeg';
    }
    ...
    if(use_number) main += '<p class="text-success">[ご利用占いメニュー]</p>';
    main += '<h4><strong>' + menu.name + '</strong></h4>';
    main += '</div>';
    main += '<div class="pt-2 center">';
    main += '<span class="pr-2 text-info">';
    if(menu.type) main += '['+getMenuType('{!!UtilYoyaku::getNewMenuSenMonTenSummary($content->service)!!}', menu.type)+']';
    main += '</span>';
    main += '<span class="text-info">[施術: ' + menu.time + '分]</span>';
    if(use_number){
        main += '<span">ご利人数: ' + use_number + '名様</span>';
    }
    main += '<br /><span class="f14 text-orange-800">&yen;' + menu.price + '</span>';
    ...
    main += truncate(menu.description, 400);
    main += '</strong></a>';
    }
    main += '</div>';
    tmpPc += main;
