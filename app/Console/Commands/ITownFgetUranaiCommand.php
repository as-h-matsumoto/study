...
class ITownFgetUranaiCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:get_itown_uranai';
    ...
    public function handle()
    {
        ...
        foreach($itownAreaNames as $area_key=>$area_name){
          ...
          while(true){
            ...
            if($area_key == 'miyagi') $loading = true;
            
            if($loading){
             //logger('go');
            }else{
             //logger('no miyagi continue');
             //logger($area_key);
              break;
            }

      
            $itonw_url = 'https://itp.ne.jp/'.$area_key.'/genre_dir/uranai/pg/'.$page_number.'/?ngr=1&nad=1&sr=1&evdc=1&sr=1&num=50';
           //logger($itonw_url);
            ...
            foreach($shop_urls as $shop_url){
             ...
             //logger($url_shop_desc);
              $choice_proxy = $proxy_list[rand(1, 60)];
              stream_context_set_default(
                array(
                 'http' => array(
                  'proxy' => $choice_proxy,
