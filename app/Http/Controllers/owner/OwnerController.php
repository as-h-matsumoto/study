...
public function postPay(Request $request)
{
  ...
  if($owner_pay = Owner_pay::where('user_id', Utilowner::getOwnerId())->first()){
    return ['err'=>1, 'message'=>'申請済みです。'];
  }
  $owner_pay = new Owner_pay;
  $owner_pay->user_id = Utilowner::getOwnerId();
  $owner_pay->status = 0;
  $owner_pay->first = 1;
  $owner_pay->save();

  $user = User::select('name','email')->find(Utilowner::getOwnerId());
  $data = array(
    'user' => $user
  );
  Mail::send('emails.owner.pay.request', $data, function ($m) use ($user) {
    $m->from('support@coordiy.com', '[Coordiy予約][ネット決済]');
    $m->to($user->email, $user->name);
    $m->bcc('support@coordiy.com', '[Coordiy予約][ネット決済]');
    $m->subject('[Coordiy予約]ネット決済申請のご連絡です。');
  });
...
public function getPay(Request $request)
{
  ...
  $owner_pay = Owner_pay::where('user_id', Utilowner::getOwnerId())->first();
  // owner_pay->status
  //  notfind : 未申請
  //  0 : 審査中
  //  1 : 利用中
  //  9 : 審査落ち
  return View::make('owner.pay.index', compact('company','owner_pay'));
...
public function postCustomer(Request $request)
{
  ...
  if(!$owners_user = Owners_users::find((int)$request->get('owners_user_id'))) return ['err'=>1, 'message'=>'顧客が見つかりません。'];
  $owners_user->name = $request->get('name');
  $owners_user->dob = $request->get('dob');
  $owners_user->description = $request->get('description');
...
public function getSupport(Request $request)
{
  ...
  return View::make('owner.support.support', compact('company','owner_service'));
}


public function postSupportBuy(Request $request)
{

  $company = company::where('user_id',Utilowner::getOwnerId())->first();
  $user = User::find(Utilowner::getOwnerId());
  $support_type = (int)$request->get('support_type');

  if($support_type===21){
    $this->netPayMail($company, $user);
  }else{
    $support_public = Util::ownerSupportTypePublic(null,'desc');
    $support_spot = Util::ownerSupportTypeSpot(null,'key');
    $this->supportBuyMail($company, $user, $support_public, $support_spot);
  }

  //if($support_type>=1 and $support_type<=4){
  //  $support_key = Util::ownerSupportTypePublic(null,'desc');
  //  //$support_desc = Util::ownerSupportTypePublic($support_type,'desc');
  //  $this->supportBuyMail($company, $user, $support_key);
  //}elseif($support_type>=11 and $support_type<=14){
  //  $support_key = Util::ownerSupportTypeSpot(null,'key');
  //  //$support_desc = Util::ownerSupportTypeSpot($support_type,'desc');
  //  $this->supportBuyMail($company, $user, $support_key);
  //}else{
  //  return ['err'=>1, 'message'=>'サポートが確認できませんでした。'];
  //}
...
function netPayMail($company, $user)
{
  ...
  Mail::send('emails.owner.pay.request', $data, function ($m) use ($user) {
    $m->from('support@coordiy.com', '[Coordiy予約]');
    $m->to($user->email, $user->name);
    $m->bcc('support@coordiy.com', '[Coordiy予約][ネット決済費用のお問合せ]');
    $m->subject('[Coordiy予約]ネット決済費用のお問合せ受付完了のご連絡です。');
  });

}

function supportBuyMail($company, $user, $support_public, $support_spot)
{
  ...
  $data = array(
    ...
    'support_spot' => $support_spot
  );
  Mail::send('emails.owner.support.request', $data, function ($m) use ($user) {
    $m->from('support@coordiy.com', '[Coordiy予約][サポート]');
    $m->to($user->email, $user->name);
    $m->bcc('support@coordiy.com', '[Coordiy予約][サポート]');
    $m->subject('[Coordiy予約]サポートお問合せ受付完了のご連絡です。');
  });
...
public function index()
{
  ...
  $company = company::where('user_id',Utilowner::getOwnerId())->first();
  $company_number = 0;
  $company_number += ($company->name) ? 10 : 0;
  $company_number += ($company->company_code) ? 10 : 0;
  $company_number += ($company->country_area_address_one) ? 10 : 0;
  $company_number += ($company->country_area_address_two) ? 10 : 0;
  $company_number += ($company->country_area_address_other) ? 10 : 0;
  $company_number += ($company->homepage) ? 10 : 0;
  $company_number += ($company->tell) ? 10 : 0;
  $company_number += ($company->description) ? 10 : 0;
  $company_number += ($company->pic) ? 10 : 0;
  $company_number += ($company->back_pic) ? 10 : 0;
  
  $support_number = 0;
  switch($company->owner_support_type){
    case 1: $support_number='ブロンズ'; break;
    case 2: $support_number='シルバー'; break;
    case 3: $support_number='ゴールド'; break;
  }
  
  if($user_bank = User_bank::select('id')->where('user_id',Utilowner::getOwnerId())->first())
  {
    $bank_number = '登録';
  }else{
    $bank_number = '未登録';
  ...
  $sellAlltime = Content_sell_month::where('owner_id',Utilowner::getOwnerId())->sum('sell');
  $sellAlltime += Content_date_users::whereIn('content_id',$content_ids)
    ->where('end','>=',$to_month_farst . ' 00:00:00')
    ->where('end','<=',$to_month_end . ' 23:59:59')
    ->sum('payment_sum');

  $customerAll = Owners_users::where('owner_id',Utilowner::getOwnerId())->count();

  $month = date("Y-m", strtotime('-1 month')) . '-01';
  if(!$send_bank = Content_sell_month::select('send_bank')
  ...
  if(!$send_bank_before = Content_sell_month::select('send_bank')
      ->where('owner_id',Utilowner::getOwnerId())
  ...
  return View('owner.index', compact('company','sellAlltime','customerAll','send_bank','send_bank_before','support_number','bank_number','company_number','contents_number'));
