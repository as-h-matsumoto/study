...
class OwnerContentsCapacityDivinationController extends Controller {
...
function validation($request, $id){
  if(!$request->get('name')) return ['err'=>1, 'message'=>'占いルーム名を入力してください。'];
  ...
  $count = Content_capacity_divination::where('content_id',$id)->count();
...
public function postCapacityNew(Request $request, $id)
{
  ...
  Utilowner::addCapacityTagdivination($capacity->content_id);
...
public function postCapacityEdit(Request $request, $id)
{
  ...
  if(!$capacity = Content_capacity_divination::find($request->get('capa_id'))) return ['err'=>1, 'message'=>'設備がみつかりません。'];
