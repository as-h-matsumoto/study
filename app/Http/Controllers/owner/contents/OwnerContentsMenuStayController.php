...
class OwnerContentsMenuStayController extends Controller {
...
public function postMenuNew(Request $request, $id)
{
  ...
  $menu = new Content_menu_stay;
  $menu->content_id = $id;
  $menu->name = $request->get('name');
  $menu->type = (int)$request->get('type');
  $menu->person = 1;
  $menu->price = (int)$request->get('price');
  $menu->number = (int)$request->get('number');
...
public function postMenuEdit(Request $request, $id)
{
  ...
  if(!$menu = Content_menu_stay::find((int)$request->get('modal-menu-id'))) return ['err'=>1, 'message'=>'メニューがみつかりません。'];
  $menu->name = $request->get('name');
  $menu->type = (int)$request->get('type');
  $menu->price = (int)$request->get('price');
  $menu->number = (int)$request->get('number');
