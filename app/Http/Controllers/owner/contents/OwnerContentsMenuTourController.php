...
class OwnerContentsMenuTourController extends Controller {
...
function validation($request, $id){
  ...
  if(!$request->get('time')) return ['err'=>1, 'message'=>'ツアー日程を入力してください。'];
...
public function postMenuEdit(Request $request, $id)
{
  ...
  if(!$menu = Content_menu_tour::find((int)$request->get('modal-menu-id'))) return ['err'=>1, 'message'=>'メニューがみつかりません。'];
  $menu->name = $request->get('name');
  $menu->type = 9;
  $menu->number = $request->get('number');
  $menu->person = ( $request->get('person') ) ? $request->get('person') : 1;
