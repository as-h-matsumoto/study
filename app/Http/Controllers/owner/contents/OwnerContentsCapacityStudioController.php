...
class OwnerContentsCapacityStudioController extends Controller {
...
function validation($request, $id){
  if(!$request->get('name')) return ['err'=>1, 'message'=>'スタジオ名を入力してください。'];
  ...
  if(!$request->get('number') ) return ['err'=>1, 'message'=>'スタジオ数を入力してください。']; 
  ...
  $count = Content_capacity_studio::where('content_id',$id)->count();
...
public function postCapacityNew(Request $request, $id)
{
  ...
  $capacity->least_time = $request->get('least_time') ? (int)$request->get('least_time') : null;
  ...
  Utilowner::addCapacityTagstudio($capacity->content_id);
...
public function postCapacityEdit(Request $request, $id)
{
  ...
  if(!$capacity = Content_capacity_studio::find($request->get('capa_id'))) return ['err'=>1, 'message'=>'設備がみつかりません。'];
