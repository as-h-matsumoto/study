...
class OwnerContentsMenuSpasalonController extends Controller {
...
function validation($request, $id){
  ...
  if(!$request->get('name')) return ['err'=>1, 'message'=>'スパメニュー名を入力してください。'];
...
public function postMenuNew(Request $request, $id)
{
  ...
  $menu = new Content_menu_spasalon;
  $menu->content_id = $id;
...
public function postMenuEdit(Request $request, $id)
{
  ...
  if(!$menu = Content_menu_spasalon::find((int)$request->get('modal-menu-id'))) return ['err'=>1, 'message'=>'メニューがみつかりません。'];
  $menu->name = $request->get('name');
  $menu->type = $request->get('type');
  $menu->simultaneously = ($request->get('simultaneously')) ? $request->get('simultaneously') : 1;
  $menu->price = $request->get('price');
