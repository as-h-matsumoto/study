...
class OwnerContentsCapacitySpasalonController extends Controller {
...
function validation($request, $id){
  if(!$request->get('name')) return ['err'=>1, 'message'=>'美容スペース名を入力してください。'];
  ...
  $count = Content_capacity_spasalon::where('content_id',$id)->count();
...
public function postCapacityNew(Request $request, $id)
{
  ...
  Utilowner::addCapacityTagspasalon($capacity->content_id);
...
public function postCapacityEdit(Request $request, $id)
{
  ...
  if(!$capacity = Content_capacity_spasalon::find($request->get('capa_id'))) return ['err'=>1, 'message'=>'設備がみつかりません。'];
