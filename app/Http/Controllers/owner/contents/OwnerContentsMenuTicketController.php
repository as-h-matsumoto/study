...
class OwnerContentsMenuTicketController extends Controller {
...
function validation($request, $id){
  ...
  if(!$request->get('name')) return ['err'=>1, 'message'=>'チケット名を入力してください。'];
  if(!$request->get('price')) return ['err'=>1, 'message'=>'料金を入力してください。'];
  if(!$request->get('number')) return ['err'=>1, 'message'=>'チケット枚数を入力してください。'];
  if(!$request->get('time')) return ['err'=>1, 'message'=>'開催時間を入力してください。'];
  if( $request->get('price') > 99999999 ) return ['err'=>1, 'message'=> '料金が大きすぎます。']; 
  if( $request->get('number') > 999999 ) return ['err'=>1, 'message'=> 'チケット枚数が大きすぎます。']; 
...
public function postMenuEdit(Request $request, $id)
{
  ...
  if(!$menu = Content_menu_ticket::find((int)$request->get('modal-menu-id'))) return ['err'=>1, 'message'=>'メニューがみつかりません。'];
  $menu->name = $request->get('name');
  $menu->type = 9;
  $menu->number = $request->get('number');
  $menu->person = ( $request->get('person') ) ? $request->get('person') : 1;
