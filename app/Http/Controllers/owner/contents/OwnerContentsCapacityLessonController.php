...
class OwnerContentsCapacityLessonController extends Controller {
...
function validation($request, $id){
  ...
  $count = Content_capacity_lesson::where('content_id',$id)->count();
...
public function postCapacityNew(Request $request, $id)
{
  ...
  Utilowner::addCapacityTaglesson($capacity->content_id);
...
public function postCapacityEdit(Request $request, $id)
{
  ...
  if(!$capacity = Content_capacity_lesson::find($request->get('capa_id'))) return ['err'=>1, 'message'=>'設備がみつかりません。'];
